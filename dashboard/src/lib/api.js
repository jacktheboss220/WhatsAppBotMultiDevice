...
export const updateGroup = (jid, update) => api.patch(`/api/admin/groups/${encodeURIComponent(jid)}`, update);
export const getGroupChatHistory = (jid, hours = 24) =>
	api.get(`/api/admin/groups/${encodeURIComponent(jid)}/chat-history?hours=${hours}`);

// Members
export const getMembers = (params) => api.get(`/api/admin/members?${new URLSearchParams(params)}`);
export const memberAction = (jid, action) => api.patch(`/api/admin/members/${encodeURIComponent(jid)}`, { action });
...
// Broadcast
export const broadcast = (message, targetJids) => api.post("/api/admin/broadcast", { message, targetJids });

// Pairing code login
export const requestPair = (phoneNumber) => api.post("/api/admin/request-pair", { phoneNumber });

// Clear WhatsApp auth (forces re-login)
export const clearAuth = () => api.post("/api/admin/clear-auth");

// Logout bot from WhatsApp (sends proper logout signal)
export const logoutBot = () => api.post("/api/admin/logout-bot");

// Reconnect — spins up a fresh socket (use after logout or clear-auth)
export const reconnectBot = () => api.post("/api/admin/reconnect");

// Full process restart (last resort — use when reconnect alone isn't enough)
export const restartBot = () => api.post("/api/admin/restart");
