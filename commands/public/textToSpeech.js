...
const handler = async (sock, msg, from, args, msgInfoObj) => {
	...
	if (!args[0] && !msg.message.extendedTextMessage)
		return sendMessageWTyping(from, { text: `❎ Text is empty! \nSend ${prefix}say text` }, { quoted: msg });

	if (!content.includes("conversation"))
		return sendMessageWTyping(from, { text: `❎ Reply to a message!` }, { quoted: msg });
	...
	if (message.split("").length >= 200)
		return sendMessageWTyping(
			from,
			{ text: `❎ Word Limit: ${message.split("").length}/200 \nSend ${prefix}say text` },
			{ quoted: msg }
		);
